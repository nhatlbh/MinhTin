...
{
    public partial class LoginSessionDT
    {
        string TableName = "Login_Session";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Login_Session.ID,Login_Session.UserID,Login_Session.SessionID,Login_Session.Token,Login_Session.Expired 
                             , Adm_User.Name as Adm_User_Name
                            FROM " + TableName +   @" Login_Session   Left Join Adm_User on  Login_Session.UserID = Adm_User.ID 
			  "
                            + ") AS grd_Login_Session  WHERE 1=1";
        ...
        {
            string query = @"SELECT Login_Session.ID,Login_Session.UserID,Login_Session.SessionID,Login_Session.Token,Login_Session.Expired FROM Login_Session WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Login_Session.ID,Login_Session.UserID,Login_Session.SessionID,Login_Session.Token,Login_Session.Expired FROM Login_Session WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(LoginSessionContract loginsession, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Login_Session
                                        VALUES ({0},{1},{2},{3})",
								
								(loginsession.Userid != null? loginsession.Userid.ToString() : "null").ToString(),
								(loginsession.Sessionid.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( loginsession.Sessionid.ToString() ) + "'" : "null").ToString(),
								(loginsession.Token.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( loginsession.Token.ToString() ) + "'" : "null").ToString(),
								(loginsession.Expired != null? "'" + SQLHelper.RejectInjection( loginsession.Expired.ToString() ) + "'" : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(LoginSessionContract loginsession, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET UserID={0},SessionID={1},Token={2},Expired={3}
                            WHERE ID=" + loginsession.Id.ToString(),
								(loginsession.Userid != null? loginsession.Userid.ToString() : "null").ToString(),
								(loginsession.Sessionid.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( loginsession.Sessionid.ToString() ) + "'" : "null").ToString(),
								(loginsession.Token.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( loginsession.Token.ToString() ) + "'" : "null").ToString(),
								(loginsession.Expired != null? "'" + SQLHelper.RejectInjection( loginsession.Expired.ToString() ) + "'" : "null").ToString());                                               
            ...
            return loginsession.Id.ToString();
        }
