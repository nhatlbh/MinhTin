...
namespace TnHSell.DT
{
    ...
    {
        string TableName = "Cat_Product_Price";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Cat_Product_Price.ID,Cat_Product_Price.ProductID,Cat_Product_Price.IOCodeID,Cat_Product_Price.Price 
                             , Cat_IOCode.Name as Cat_IOCode_Name, Cat_Product.Name as Cat_Product_Name
                            FROM " + TableName +   @" Cat_Product_Price   Left Join Cat_IOCode on  Cat_Product_Price.IOCodeID = Cat_IOCode.ID 
			 Left Join Cat_Product on  Cat_Product_Price.ProductID = Cat_Product.ID 
			  "
                            + ") AS grd_Cat_Product_Price  WHERE 1=1";
        ...
        {
            string query = @"SELECT Cat_Product_Price.ID,Cat_Product_Price.ProductID,Cat_Product_Price.IOCodeID,Cat_Product_Price.Price FROM Cat_Product_Price WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Cat_Product_Price.ID,Cat_Product_Price.ProductID,Cat_Product_Price.IOCodeID,Cat_Product_Price.Price FROM Cat_Product_Price WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(CatProductPriceContract catproductprice, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Cat_Product_Price
                                        VALUES ({0},{1},{2})",
								
								(catproductprice.Productid != null? catproductprice.Productid.ToString() : "null").ToString(),
								(catproductprice.Iocodeid != null? catproductprice.Iocodeid.ToString() : "null").ToString(),
								(catproductprice.Price.Trim() != String.Empty ? catproductprice.Price.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(CatProductPriceContract catproductprice, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET ProductID={0},IOCodeID={1},Price={2}
                            WHERE ID=" + catproductprice.Id.ToString(),
								(catproductprice.Productid != null? catproductprice.Productid.ToString() : "null").ToString(),
								(catproductprice.Iocodeid != null? catproductprice.Iocodeid.ToString() : "null").ToString(),
								(catproductprice.Price.Trim() != String.Empty ? catproductprice.Price.ToString() : "null").ToString());                                               
            if (tran == null)
            ...
            return catproductprice.Id.ToString();
        }
