...
{
    public partial class CatProvinceDT
    {
        string TableName = "Cat_Province";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Cat_Province.ID,Cat_Province.Code,Cat_Province.Name,Cat_Province.Description,Cat_Province.OrderNum 
                            
                            FROM " + TableName +   @" Cat_Province    "
                            + ") AS grd_Cat_Province  WHERE 1=1";
        ...
        {
            string query = @"SELECT Cat_Province.ID,Cat_Province.Code,Cat_Province.Name,Cat_Province.Description,Cat_Province.OrderNum FROM Cat_Province WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Cat_Province.ID,Cat_Province.Code,Cat_Province.Name,Cat_Province.Description,Cat_Province.OrderNum FROM Cat_Province WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(CatProvinceContract catprovince, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Cat_Province
                                        VALUES ({0},{1},{2},{3})",
								
								(catprovince.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catprovince.Code.ToString() ) + "'" : "null").ToString(),
								(catprovince.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catprovince.Name.ToString() ) + "'" : "null").ToString(),
								(catprovince.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catprovince.Description.ToString() ) + "'" : "null").ToString(),
								(catprovince.Ordernum.Trim() != String.Empty ? catprovince.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(CatProvinceContract catprovince, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},Description={2},OrderNum={3}
                            WHERE ID=" + catprovince.Id.ToString(),
								(catprovince.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catprovince.Code.ToString() ) + "'" : "null").ToString(),
								(catprovince.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catprovince.Name.ToString() ) + "'" : "null").ToString(),
								(catprovince.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catprovince.Description.ToString() ) + "'" : "null").ToString(),
								(catprovince.Ordernum.Trim() != String.Empty ? catprovince.Ordernum.ToString() : "null").ToString());                                               
            ...
            return catprovince.Id.ToString();
        }
