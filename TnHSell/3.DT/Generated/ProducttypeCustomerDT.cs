...
{
    public partial class ProducttypeCustomerDT
    {
        string TableName = "ProductType_Customer";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            ProductType_Customer.ID,ProductType_Customer.ProductTypeID,ProductType_Customer.CustomerID,convert(varchar(10),ProductType_Customer.GetinDate,103) as GetinDate 
                             , Cat_ProductType.Name as Cat_ProductType_Name, Cat_Customer.Name as Cat_Customer_Name
                            FROM " + TableName +   @" ProductType_Customer   Left Join Cat_ProductType on  ProductType_Customer.ProductTypeID = Cat_ProductType.ID 
			 Left Join Cat_Customer on  ProductType_Customer.CustomerID = Cat_Customer.ID 
			  "
                            + ") AS grd_ProductType_Customer  WHERE 1=1";
        ...
        {
            string query = @"SELECT ProductType_Customer.ID,ProductType_Customer.ProductTypeID,ProductType_Customer.CustomerID,convert(varchar(10),ProductType_Customer.GetinDate,103) as GetinDate FROM ProductType_Customer WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT ProductType_Customer.ID,ProductType_Customer.ProductTypeID,ProductType_Customer.CustomerID,convert(varchar(10),ProductType_Customer.GetinDate,103) as GetinDate FROM ProductType_Customer WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
