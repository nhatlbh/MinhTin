...
{
    public partial class AdmServiceDT
    {
        string TableName = "Adm_Service";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Adm_Service.ID,Adm_Service.Code,Adm_Service.Name,Adm_Service.OrderNum 
                            
                            FROM " + TableName +   @" Adm_Service    "
                            + ") AS grd_Adm_Service  WHERE 1=1";
        ...
        {
            string query = @"SELECT Adm_Service.ID,Adm_Service.Code,Adm_Service.Name,Adm_Service.OrderNum FROM Adm_Service WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Adm_Service.ID,Adm_Service.Code,Adm_Service.Name,Adm_Service.OrderNum FROM Adm_Service WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(AdmServiceContract admservice, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Adm_Service
                                        VALUES ({0},{1},{2})",
								
								(admservice.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admservice.Code.ToString() ) + "'" : "null").ToString(),
								(admservice.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admservice.Name.ToString() ) + "'" : "null").ToString(),
								(admservice.Ordernum.Trim() != String.Empty ? admservice.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(AdmServiceContract admservice, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},OrderNum={2}
                            WHERE ID=" + admservice.Id.ToString(),
								(admservice.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admservice.Code.ToString() ) + "'" : "null").ToString(),
								(admservice.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admservice.Name.ToString() ) + "'" : "null").ToString(),
								(admservice.Ordernum.Trim() != String.Empty ? admservice.Ordernum.ToString() : "null").ToString());                                               
            if (tran == null)
