...
{
    public partial class AdmUserroleDT
    {
        string TableName = "Adm_UserRole";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Adm_UserRole.ID,Adm_UserRole.UserID,Adm_UserRole.RoleID 
                             , Adm_Role.Name as Adm_Role_Name, Adm_User.Name as Adm_User_Name
                            FROM " + TableName +   @" Adm_UserRole   Left Join Adm_Role on  Adm_UserRole.RoleID = Adm_Role.ID 
			 Left Join Adm_User on  Adm_UserRole.UserID = Adm_User.ID 
			  "
                            + ") AS grd_Adm_UserRole  WHERE 1=1";
        ...
        {
            string query = @"SELECT Adm_UserRole.ID,Adm_UserRole.UserID,Adm_UserRole.RoleID FROM Adm_UserRole WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Adm_UserRole.ID,Adm_UserRole.UserID,Adm_UserRole.RoleID FROM Adm_UserRole WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
