...
{
    public partial class CatChanelDT
    {
        string TableName = "Cat_Chanel";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Cat_Chanel.ID,Cat_Chanel.Code,Cat_Chanel.Name,Cat_Chanel.Description,Cat_Chanel.OrderNum 
                            
                            FROM " + TableName +   @" Cat_Chanel    "
                            + ") AS grd_Cat_Chanel  WHERE 1=1";
        ...
        {
            string query = @"SELECT Cat_Chanel.ID,Cat_Chanel.Code,Cat_Chanel.Name,Cat_Chanel.Description,Cat_Chanel.OrderNum FROM Cat_Chanel WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Cat_Chanel.ID,Cat_Chanel.Code,Cat_Chanel.Name,Cat_Chanel.Description,Cat_Chanel.OrderNum FROM Cat_Chanel WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(CatChanelContract catchanel, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Cat_Chanel
                                        VALUES ({0},{1},{2},{3})",
								
								(catchanel.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catchanel.Code.ToString() ) + "'" : "null").ToString(),
								(catchanel.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catchanel.Name.ToString() ) + "'" : "null").ToString(),
								(catchanel.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catchanel.Description.ToString() ) + "'" : "null").ToString(),
								(catchanel.Ordernum.Trim() != String.Empty ? catchanel.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(CatChanelContract catchanel, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},Description={2},OrderNum={3}
                            WHERE ID=" + catchanel.Id.ToString(),
								(catchanel.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catchanel.Code.ToString() ) + "'" : "null").ToString(),
								(catchanel.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catchanel.Name.ToString() ) + "'" : "null").ToString(),
								(catchanel.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catchanel.Description.ToString() ) + "'" : "null").ToString(),
								(catchanel.Ordernum.Trim() != String.Empty ? catchanel.Ordernum.ToString() : "null").ToString());                                               
