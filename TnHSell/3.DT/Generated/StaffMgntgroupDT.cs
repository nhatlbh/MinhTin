...
{
    public partial class StaffMgntgroupDT
    {
        string TableName = "Staff_MgntGroup";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Staff_MgntGroup.ID,Staff_MgntGroup.SaleStaffID,Staff_MgntGroup.ManagementGroupID 
                             , Cat_ManagementGroup.Name as Cat_ManagementGroup_Name, Cat_SaleStaff.Name as Cat_SaleStaff_Name
                            FROM " + TableName +   @" Staff_MgntGroup   Left Join Cat_ManagementGroup on  Staff_MgntGroup.ManagementGroupID = Cat_ManagementGroup.ID 
			 Left Join Cat_SaleStaff on  Staff_MgntGroup.SaleStaffID = Cat_SaleStaff.ID 
			  "
                            + ") AS grd_Staff_MgntGroup  WHERE 1=1";
        ...
        {
            string query = @"SELECT Staff_MgntGroup.ID,Staff_MgntGroup.SaleStaffID,Staff_MgntGroup.ManagementGroupID FROM Staff_MgntGroup WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Staff_MgntGroup.ID,Staff_MgntGroup.SaleStaffID,Staff_MgntGroup.ManagementGroupID FROM Staff_MgntGroup WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(StaffMgntgroupContract staffmgntgroup, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Staff_MgntGroup
                                        VALUES ({0},{1})",
								
								(staffmgntgroup.Salestaffid != null? staffmgntgroup.Salestaffid.ToString() : "null").ToString(),
								(staffmgntgroup.Managementgroupid != null? staffmgntgroup.Managementgroupid.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(StaffMgntgroupContract staffmgntgroup, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET SaleStaffID={0},ManagementGroupID={1}
                            WHERE ID=" + staffmgntgroup.Id.ToString(),
								(staffmgntgroup.Salestaffid != null? staffmgntgroup.Salestaffid.ToString() : "null").ToString(),
								(staffmgntgroup.Managementgroupid != null? staffmgntgroup.Managementgroupid.ToString() : "null").ToString());                                               
            if (tran == null)
            {
            ...
            return staffmgntgroup.Id.ToString();
        }
