...
{
    public partial class CatManufactureDT
    {
        string TableName = "Cat_Manufacture";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Cat_Manufacture.ID,Cat_Manufacture.Code,Cat_Manufacture.Name,Cat_Manufacture.Description,Cat_Manufacture.OrderNum 
                            
                            FROM " + TableName +   @" Cat_Manufacture    "
                            + ") AS grd_Cat_Manufacture  WHERE 1=1";
        ...
        {
            string query = @"SELECT Cat_Manufacture.ID,Cat_Manufacture.Code,Cat_Manufacture.Name,Cat_Manufacture.Description,Cat_Manufacture.OrderNum FROM Cat_Manufacture WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Cat_Manufacture.ID,Cat_Manufacture.Code,Cat_Manufacture.Name,Cat_Manufacture.Description,Cat_Manufacture.OrderNum FROM Cat_Manufacture WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(CatManufactureContract catmanufacture, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Cat_Manufacture
                                        VALUES ({0},{1},{2},{3})",
								
								(catmanufacture.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catmanufacture.Code.ToString() ) + "'" : "null").ToString(),
								(catmanufacture.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catmanufacture.Name.ToString() ) + "'" : "null").ToString(),
								(catmanufacture.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catmanufacture.Description.ToString() ) + "'" : "null").ToString(),
								(catmanufacture.Ordernum.Trim() != String.Empty ? catmanufacture.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(CatManufactureContract catmanufacture, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},Description={2},OrderNum={3}
                            WHERE ID=" + catmanufacture.Id.ToString(),
								(catmanufacture.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catmanufacture.Code.ToString() ) + "'" : "null").ToString(),
								(catmanufacture.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catmanufacture.Name.ToString() ) + "'" : "null").ToString(),
								(catmanufacture.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catmanufacture.Description.ToString() ) + "'" : "null").ToString(),
								(catmanufacture.Ordernum.Trim() != String.Empty ? catmanufacture.Ordernum.ToString() : "null").ToString());                                               
            ...
            return catmanufacture.Id.ToString();
        }
