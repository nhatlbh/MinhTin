...
{
    public partial class AdmContextDT
    {
        string TableName = "Adm_Context";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Adm_Context.ID,Adm_Context.Code,Adm_Context.Name,Adm_Context.DevName,Adm_Context.MapID,Adm_Context.Type,Adm_Context.RootMap,Adm_Context.OrderNum 
                             , Adm_Map.Name as Adm_Map_Name
                            FROM " + TableName +   @" Adm_Context   Left Join Adm_Map on  Adm_Context.MapID = Adm_Map.ID 
			  "
                            + ") AS grd_Adm_Context  WHERE 1=1";
        ...
        {
            string query = @"SELECT Adm_Context.ID,Adm_Context.Code,Adm_Context.Name,Adm_Context.DevName,Adm_Context.MapID,Adm_Context.Type,Adm_Context.RootMap,Adm_Context.OrderNum FROM Adm_Context WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Adm_Context.ID,Adm_Context.Code,Adm_Context.Name,Adm_Context.DevName,Adm_Context.MapID,Adm_Context.Type,Adm_Context.RootMap,Adm_Context.OrderNum FROM Adm_Context WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(AdmContextContract admcontext, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Adm_Context
                                        VALUES ({0},{1},{2},{3},{4},{5},{6})",
								
								(admcontext.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admcontext.Code.ToString() ) + "'" : "null").ToString(),
								(admcontext.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admcontext.Name.ToString() ) + "'" : "null").ToString(),
								(admcontext.Devname.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admcontext.Devname.ToString() ) + "'" : "null").ToString(),
								(admcontext.Mapid != null? admcontext.Mapid.ToString() : "null").ToString(),
								(admcontext.Type.Trim() != String.Empty ? admcontext.Type.ToString() : "null").ToString(),
								(admcontext.Rootmap.Trim() != String.Empty ? admcontext.Rootmap.ToString() : "null").ToString(),
								(admcontext.Ordernum.Trim() != String.Empty ? admcontext.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(AdmContextContract admcontext, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},DevName={2},MapID={3},Type={4},RootMap={5},OrderNum={6}
                            WHERE ID=" + admcontext.Id.ToString(),
								(admcontext.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admcontext.Code.ToString() ) + "'" : "null").ToString(),
								(admcontext.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admcontext.Name.ToString() ) + "'" : "null").ToString(),
								(admcontext.Devname.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admcontext.Devname.ToString() ) + "'" : "null").ToString(),
								(admcontext.Mapid != null? admcontext.Mapid.ToString() : "null").ToString(),
