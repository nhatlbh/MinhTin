...
{
    public partial class CatUnitDT
    {
        string TableName = "Cat_Unit";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Cat_Unit.ID,Cat_Unit.Code,Cat_Unit.Name,Cat_Unit.Description,Cat_Unit.OrderNum 
                            
                            FROM " + TableName +   @" Cat_Unit    "
                            + ") AS grd_Cat_Unit  WHERE 1=1";
        ...
        {
            string query = @"SELECT Cat_Unit.ID,Cat_Unit.Code,Cat_Unit.Name,Cat_Unit.Description,Cat_Unit.OrderNum FROM Cat_Unit WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Cat_Unit.ID,Cat_Unit.Code,Cat_Unit.Name,Cat_Unit.Description,Cat_Unit.OrderNum FROM Cat_Unit WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(CatUnitContract catunit, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Cat_Unit
                                        VALUES ({0},{1},{2},{3})",
								
								(catunit.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catunit.Code.ToString() ) + "'" : "null").ToString(),
								(catunit.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catunit.Name.ToString() ) + "'" : "null").ToString(),
								(catunit.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catunit.Description.ToString() ) + "'" : "null").ToString(),
								(catunit.Ordernum.Trim() != String.Empty ? catunit.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(CatUnitContract catunit, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},Description={2},OrderNum={3}
                            WHERE ID=" + catunit.Id.ToString(),
								(catunit.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catunit.Code.ToString() ) + "'" : "null").ToString(),
								(catunit.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catunit.Name.ToString() ) + "'" : "null").ToString(),
								(catunit.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catunit.Description.ToString() ) + "'" : "null").ToString(),
								(catunit.Ordernum.Trim() != String.Empty ? catunit.Ordernum.ToString() : "null").ToString());                                               
