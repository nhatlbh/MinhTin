    ...
    public partial class CatBranchDT
    {
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Cat_Branch.ID,Cat_Branch.Code,Cat_Branch.Name,Cat_Branch.ParentBranchID,Cat_Branch.Description,Cat_Branch.Address,Cat_Branch.Phone,Cat_Branch.OrderNum 
                             , Cat_Branch1.Name as Cat_Branch_Name
                            FROM " + TableName +   @" Cat_Branch   Left Join Cat_Branch Cat_Branch1 on  Cat_Branch.ParentBranchID = Cat_Branch1.ID 
			  "
                            + ") AS grd_Cat_Branch  WHERE 1=1";
        ...
        {
            string query = @"SELECT Cat_Branch.ID,Cat_Branch.Code,Cat_Branch.Name,Cat_Branch.ParentBranchID,Cat_Branch.Description,Cat_Branch.Address,Cat_Branch.Phone,Cat_Branch.OrderNum FROM Cat_Branch WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Cat_Branch.ID,Cat_Branch.Code,Cat_Branch.Name,Cat_Branch.ParentBranchID,Cat_Branch.Description,Cat_Branch.Address,Cat_Branch.Phone,Cat_Branch.OrderNum FROM Cat_Branch WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(CatBranchContract catbranch, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Cat_Branch
                                        VALUES ({0},{1},{2},{3},{4},{5},{6})",
								
								(catbranch.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Code.ToString() ) + "'" : "null").ToString(),
								(catbranch.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Name.ToString() ) + "'" : "null").ToString(),
								(catbranch.Parentbranchid != null? catbranch.Parentbranchid.ToString() : "null").ToString(),
								(catbranch.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Description.ToString() ) + "'" : "null").ToString(),
								(catbranch.Address.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Address.ToString() ) + "'" : "null").ToString(),
								(catbranch.Phone.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Phone.ToString() ) + "'" : "null").ToString(),
								(catbranch.Ordernum.Trim() != String.Empty ? catbranch.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(CatBranchContract catbranch, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},ParentBranchID={2},Description={3},Address={4},Phone={5},OrderNum={6}
                            WHERE ID=" + catbranch.Id.ToString(),
								(catbranch.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Code.ToString() ) + "'" : "null").ToString(),
								(catbranch.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Name.ToString() ) + "'" : "null").ToString(),
								(catbranch.Parentbranchid != null? catbranch.Parentbranchid.ToString() : "null").ToString(),
								(catbranch.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catbranch.Description.ToString() ) + "'" : "null").ToString(),
