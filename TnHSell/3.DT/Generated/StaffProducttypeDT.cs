...
{
    public partial class StaffProducttypeDT
    {
        string TableName = "Staff_ProductType";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Staff_ProductType.ID,Staff_ProductType.ProductTypeID,Staff_ProductType.SaleStaffID,Staff_ProductType.SeeAll 
                             , Cat_SaleStaff.Name as Cat_SaleStaff_Name, Cat_ProductType.Name as Cat_ProductType_Name
                            FROM " + TableName +   @" Staff_ProductType   Left Join Cat_SaleStaff on  Staff_ProductType.SaleStaffID = Cat_SaleStaff.ID 
			 Left Join Cat_ProductType on  Staff_ProductType.ProductTypeID = Cat_ProductType.ID 
			  "
                            + ") AS grd_Staff_ProductType  WHERE 1=1";
        ...
        {
            string query = @"SELECT Staff_ProductType.ID,Staff_ProductType.ProductTypeID,Staff_ProductType.SaleStaffID,Staff_ProductType.SeeAll FROM Staff_ProductType WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Staff_ProductType.ID,Staff_ProductType.ProductTypeID,Staff_ProductType.SaleStaffID,Staff_ProductType.SeeAll FROM Staff_ProductType WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(StaffProducttypeContract staffproducttype, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Staff_ProductType
                                        VALUES ({0},{1},{2})",
								
								(staffproducttype.Producttypeid != null? staffproducttype.Producttypeid.ToString() : "null").ToString(),
								(staffproducttype.Salestaffid != null? staffproducttype.Salestaffid.ToString() : "null").ToString(),
								(staffproducttype.Seeall != null? "'" + SQLHelper.RejectInjection( staffproducttype.Seeall.ToString() ) + "'" : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(StaffProducttypeContract staffproducttype, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET ProductTypeID={0},SaleStaffID={1},SeeAll={2}
                            WHERE ID=" + staffproducttype.Id.ToString(),
								(staffproducttype.Producttypeid != null? staffproducttype.Producttypeid.ToString() : "null").ToString(),
								(staffproducttype.Salestaffid != null? staffproducttype.Salestaffid.ToString() : "null").ToString(),
								(staffproducttype.Seeall != null? "'" + SQLHelper.RejectInjection( staffproducttype.Seeall.ToString() ) + "'" : "null").ToString());                                               
            if (tran == null)
            ...
            return staffproducttype.Id.ToString();
        }
