...
{
    public partial class CatIocodeDT
    {
        string TableName = "Cat_IOCode";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Cat_IOCode.ID,Cat_IOCode.Code,Cat_IOCode.Name,Cat_IOCode.Description,Cat_IOCode.OrderNum 
                            
                            FROM " + TableName +   @" Cat_IOCode    "
                            + ") AS grd_Cat_IOCode  WHERE 1=1";
        ...
        {
            string query = @"SELECT Cat_IOCode.ID,Cat_IOCode.Code,Cat_IOCode.Name,Cat_IOCode.Description,Cat_IOCode.OrderNum FROM Cat_IOCode WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Cat_IOCode.ID,Cat_IOCode.Code,Cat_IOCode.Name,Cat_IOCode.Description,Cat_IOCode.OrderNum FROM Cat_IOCode WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(CatIocodeContract catiocode, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Cat_IOCode
                                        VALUES ({0},{1},{2},{3})",
								
								(catiocode.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catiocode.Code.ToString() ) + "'" : "null").ToString(),
								(catiocode.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catiocode.Name.ToString() ) + "'" : "null").ToString(),
								(catiocode.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catiocode.Description.ToString() ) + "'" : "null").ToString(),
								(catiocode.Ordernum.Trim() != String.Empty ? catiocode.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(CatIocodeContract catiocode, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},Description={2},OrderNum={3}
                            WHERE ID=" + catiocode.Id.ToString(),
								(catiocode.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catiocode.Code.ToString() ) + "'" : "null").ToString(),
								(catiocode.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catiocode.Name.ToString() ) + "'" : "null").ToString(),
								(catiocode.Description.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( catiocode.Description.ToString() ) + "'" : "null").ToString(),
								(catiocode.Ordernum.Trim() != String.Empty ? catiocode.Ordernum.ToString() : "null").ToString());                                               
