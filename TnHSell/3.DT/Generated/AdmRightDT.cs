...
{
    public partial class AdmRightDT
    {
        string TableName = "Adm_Right";
        /// <summary>
        ...
        public DataTable GetGridData(string cond, string order="", SqlTransaction  tran = null )
        {
            string query = @"SELECT * FROM (
                            SELECT 
                            Adm_Right.ID,Adm_Right.Code,Adm_Right.Name,Adm_Right.OrderNum 
                            
                            FROM " + TableName +   @" Adm_Right    "
                            + ") AS grd_Adm_Right  WHERE 1=1";
        ...
        {
            string query = @"SELECT Adm_Right.ID,Adm_Right.Code,Adm_Right.Name,Adm_Right.OrderNum FROM Adm_Right WHERE ID = " + id;
            DataTable dt = DataProvider.ExecuteQuery(query);
            return dt;
        }
        /// <summary>
        ...
        {
            string query = @"SELECT Adm_Right.ID,Adm_Right.Code,Adm_Right.Name,Adm_Right.OrderNum FROM Adm_Right WHERE 1=1 ";
            if (cond != null && cond != string.Empty)
            {
                query += " and " + cond;
            };
        ...
        /// <returns></returns>
        public string Insert(AdmRightContract admright, SqlTransaction  tran = null)
        {
            DataTable dtResult = null;
            string query = string.Format(@"INSERT INTO Adm_Right
                                        VALUES ({0},{1},{2})",
								
								(admright.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admright.Code.ToString() ) + "'" : "null").ToString(),
								(admright.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admright.Name.ToString() ) + "'" : "null").ToString(),
								(admright.Ordernum.Trim() != String.Empty ? admright.Ordernum.ToString() : "null").ToString());
        ...
        /// <returns></returns>
        public string Update(AdmRightContract admright, SqlTransaction  tran = null)
        {
            string query = String.Format("UPDATE " + TableName + @" SET Code={0},Name={1},OrderNum={2}
                            WHERE ID=" + admright.Id.ToString(),
								(admright.Code.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admright.Code.ToString() ) + "'" : "null").ToString(),
								(admright.Name.Trim() != String.Empty? "N'" + SQLHelper.RejectInjection( admright.Name.ToString() ) + "'" : "null").ToString(),
								(admright.Ordernum.Trim() != String.Empty ? admright.Ordernum.ToString() : "null").ToString());                                               
            if (tran == null)
