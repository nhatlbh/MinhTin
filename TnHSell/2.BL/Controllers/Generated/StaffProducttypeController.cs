...
{
   public partial class StaffProducttypeController: ApiController
    {
        StaffProducttypeDT dta = new StaffProducttypeDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(StaffProducttypeBR));
        [Route("StaffProducttype/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("StaffProducttype/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(StaffProducttypeDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("StaffProducttype/GetByID")]
        [HttpGet, HttpPost]
        ...
        [Route("StaffProducttype/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string staffproducttypeJson)
        {
          string errMessage = string.Empty;
          try
            {  StaffProducttypeContract staffproducttype = JsonConvert.DeserializeObject<StaffProducttypeContract>(staffproducttypeJson);           
                businessRule.RegistInstants(staffproducttype);
                if (staffproducttype.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(staffproducttype));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(staffproducttype));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("StaffProducttype/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = StaffProducttypeDFR.GetComboboxData(sessionKey,out columns,cond, out order);
