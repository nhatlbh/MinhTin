...
{
   public partial class LoginSessionController: ApiController
    {
        LoginSessionDT dta = new LoginSessionDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(LoginSessionBR));
        [Route("LoginSession/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("LoginSession/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(LoginSessionDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("LoginSession/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string loginsessionJson)
        {
          string errMessage = string.Empty;
          try
            {  LoginSessionContract loginsession = JsonConvert.DeserializeObject<LoginSessionContract>(loginsessionJson);           
                businessRule.RegistInstants(loginsession);
                if (loginsession.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(loginsession));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(loginsession));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("LoginSession/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = LoginSessionDFR.GetComboboxData(sessionKey,out columns,cond, out order);
