...
{
   public partial class CatChanelController: ApiController
    {
        CatChanelDT dta = new CatChanelDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatChanelBR));
        [Route("CatChanel/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatChanel/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatChanelDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatChanel/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catchanelJson)
        {
          string errMessage = string.Empty;
          try
            {  CatChanelContract catchanel = JsonConvert.DeserializeObject<CatChanelContract>(catchanelJson);           
                businessRule.RegistInstants(catchanel);
                if (catchanel.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catchanel));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catchanel));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatChanel/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatChanelDFR.GetComboboxData(sessionKey,out columns,cond, out order);
