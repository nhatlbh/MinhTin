...
{
   public partial class AdmUserController: ApiController
    {
        AdmUserDT dta = new AdmUserDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(AdmUserBR));
        [Route("AdmUser/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("AdmUser/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(AdmUserDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = AdmUserDFR.GetComboboxData(sessionKey,out columns,cond, out order);
