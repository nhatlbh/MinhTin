...
{
   public partial class CatProductController: ApiController
    {
        CatProductDT dta = new CatProductDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatProductBR));
        [Route("CatProduct/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatProduct/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatProductDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatProduct/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catproductJson)
        {
          string errMessage = string.Empty;
          try
            {  CatProductContract catproduct = JsonConvert.DeserializeObject<CatProductContract>(catproductJson);           
                businessRule.RegistInstants(catproduct);
                if (catproduct.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catproduct));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catproduct));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatProduct/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatProductDFR.GetComboboxData(sessionKey,out columns,cond, out order);
