...
{
   public partial class SelInvoiceController: ApiController
    {
        SelInvoiceDT dta = new SelInvoiceDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(SelInvoiceBR));
        [Route("SelInvoice/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("SelInvoice/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(SelInvoiceDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("SelInvoice/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string selinvoiceJson)
        {
          string errMessage = string.Empty;
          try
            {  SelInvoiceContract selinvoice = JsonConvert.DeserializeObject<SelInvoiceContract>(selinvoiceJson);           
                businessRule.RegistInstants(selinvoice);
                if (selinvoice.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(selinvoice));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(selinvoice));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("SelInvoice/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = SelInvoiceDFR.GetComboboxData(sessionKey,out columns,cond, out order);
