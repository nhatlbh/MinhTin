...
{
   public partial class CatBranchController: ApiController
    {
        CatBranchDT dta = new CatBranchDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatBranchBR));
        [Route("CatBranch/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatBranch/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatBranchDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatBranch/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catbranchJson)
        {
          string errMessage = string.Empty;
          try
            {  CatBranchContract catbranch = JsonConvert.DeserializeObject<CatBranchContract>(catbranchJson);           
                businessRule.RegistInstants(catbranch);
                if (catbranch.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catbranch));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catbranch));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatBranch/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatBranchDFR.GetComboboxData(sessionKey,out columns,cond, out order);
