...
{
   public partial class StaffMgntgroupController: ApiController
    {
        StaffMgntgroupDT dta = new StaffMgntgroupDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(StaffMgntgroupBR));
        [Route("StaffMgntgroup/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("StaffMgntgroup/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(StaffMgntgroupDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("StaffMgntgroup/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string staffmgntgroupJson)
        {
          string errMessage = string.Empty;
          try
            {  StaffMgntgroupContract staffmgntgroup = JsonConvert.DeserializeObject<StaffMgntgroupContract>(staffmgntgroupJson);           
                businessRule.RegistInstants(staffmgntgroup);
                if (staffmgntgroup.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(staffmgntgroup));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(staffmgntgroup));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("StaffMgntgroup/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = StaffMgntgroupDFR.GetComboboxData(sessionKey,out columns,cond, out order);
