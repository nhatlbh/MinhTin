...
{
   public partial class AdmRightController: ApiController
    {
        AdmRightDT dta = new AdmRightDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(AdmRightBR));
        [Route("AdmRight/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("AdmRight/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(AdmRightDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("AdmRight/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string admrightJson)
        {
          string errMessage = string.Empty;
          try
            {  AdmRightContract admright = JsonConvert.DeserializeObject<AdmRightContract>(admrightJson);           
                businessRule.RegistInstants(admright);
                if (admright.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(admright));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(admright));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("AdmRight/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = AdmRightDFR.GetComboboxData(sessionKey,out columns,cond, out order);
