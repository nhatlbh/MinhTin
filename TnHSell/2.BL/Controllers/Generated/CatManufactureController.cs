...
{
   public partial class CatManufactureController: ApiController
    {
        CatManufactureDT dta = new CatManufactureDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatManufactureBR));
        [Route("CatManufacture/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatManufacture/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatManufactureDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatManufacture/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catmanufactureJson)
        {
          string errMessage = string.Empty;
          try
            {  CatManufactureContract catmanufacture = JsonConvert.DeserializeObject<CatManufactureContract>(catmanufactureJson);           
                businessRule.RegistInstants(catmanufacture);
                if (catmanufacture.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catmanufacture));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catmanufacture));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatManufacture/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatManufactureDFR.GetComboboxData(sessionKey,out columns,cond, out order);
