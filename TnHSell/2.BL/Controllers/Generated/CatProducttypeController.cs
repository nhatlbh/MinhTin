...
{
   public partial class CatProducttypeController: ApiController
    {
        CatProducttypeDT dta = new CatProducttypeDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatProducttypeBR));
        [Route("CatProducttype/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatProducttype/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatProducttypeDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatProducttype/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catproducttypeJson)
        {
          string errMessage = string.Empty;
          try
            {  CatProducttypeContract catproducttype = JsonConvert.DeserializeObject<CatProducttypeContract>(catproducttypeJson);           
                businessRule.RegistInstants(catproducttype);
                if (catproducttype.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catproducttype));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catproducttype));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatProducttype/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatProducttypeDFR.GetComboboxData(sessionKey,out columns,cond, out order);
