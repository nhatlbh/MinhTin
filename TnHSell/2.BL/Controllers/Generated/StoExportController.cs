...
{
   public partial class StoExportController: ApiController
    {
        StoExportDT dta = new StoExportDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(StoExportBR));
        [Route("StoExport/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("StoExport/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(StoExportDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("StoExport/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string stoexportJson)
        {
          string errMessage = string.Empty;
          try
            {  StoExportContract stoexport = JsonConvert.DeserializeObject<StoExportContract>(stoexportJson);           
                businessRule.RegistInstants(stoexport);
                if (stoexport.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(stoexport));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(stoexport));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("StoExport/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = StoExportDFR.GetComboboxData(sessionKey,out columns,cond, out order);
