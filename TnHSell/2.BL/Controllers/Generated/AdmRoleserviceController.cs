...
{
   public partial class AdmRoleserviceController: ApiController
    {
        AdmRoleserviceDT dta = new AdmRoleserviceDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(AdmRoleserviceBR));
        [Route("AdmRoleservice/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("AdmRoleservice/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(AdmRoleserviceDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("AdmRoleservice/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string admroleserviceJson)
        {
          string errMessage = string.Empty;
          try
            {  AdmRoleserviceContract admroleservice = JsonConvert.DeserializeObject<AdmRoleserviceContract>(admroleserviceJson);           
                businessRule.RegistInstants(admroleservice);
                if (admroleservice.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(admroleservice));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(admroleservice));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("AdmRoleservice/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = AdmRoleserviceDFR.GetComboboxData(sessionKey,out columns,cond, out order);
