...
{
   public partial class CatProvinceController: ApiController
    {
        CatProvinceDT dta = new CatProvinceDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatProvinceBR));
        [Route("CatProvince/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatProvince/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatProvinceDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatProvince/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catprovinceJson)
        {
          string errMessage = string.Empty;
          try
            {  CatProvinceContract catprovince = JsonConvert.DeserializeObject<CatProvinceContract>(catprovinceJson);           
                businessRule.RegistInstants(catprovince);
                if (catprovince.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catprovince));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catprovince));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatProvince/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatProvinceDFR.GetComboboxData(sessionKey,out columns,cond, out order);
