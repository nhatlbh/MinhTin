...
{
   public partial class FinMoneyslipController: ApiController
    {
        FinMoneyslipDT dta = new FinMoneyslipDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(FinMoneyslipBR));
        [Route("FinMoneyslip/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("FinMoneyslip/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(FinMoneyslipDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("FinMoneyslip/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string finmoneyslipJson)
        {
          string errMessage = string.Empty;
          try
            {  FinMoneyslipContract finmoneyslip = JsonConvert.DeserializeObject<FinMoneyslipContract>(finmoneyslipJson);           
                businessRule.RegistInstants(finmoneyslip);
                if (finmoneyslip.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(finmoneyslip));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(finmoneyslip));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("FinMoneyslip/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = FinMoneyslipDFR.GetComboboxData(sessionKey,out columns,cond, out order);
