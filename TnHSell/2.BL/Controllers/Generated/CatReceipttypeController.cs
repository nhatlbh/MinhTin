...
{
   public partial class CatReceipttypeController: ApiController
    {
        CatReceipttypeDT dta = new CatReceipttypeDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatReceipttypeBR));
        [Route("CatReceipttype/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatReceipttype/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatReceipttypeDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatReceipttype/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catreceipttypeJson)
        {
          string errMessage = string.Empty;
          try
            {  CatReceipttypeContract catreceipttype = JsonConvert.DeserializeObject<CatReceipttypeContract>(catreceipttypeJson);           
                businessRule.RegistInstants(catreceipttype);
                if (catreceipttype.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catreceipttype));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catreceipttype));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatReceipttype/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatReceipttypeDFR.GetComboboxData(sessionKey,out columns,cond, out order);
