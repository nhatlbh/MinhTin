...
{
   public partial class BuyImportinvoiceController: ApiController
    {
        BuyImportinvoiceDT dta = new BuyImportinvoiceDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(BuyImportinvoiceBR));
        [Route("BuyImportinvoice/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("BuyImportinvoice/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(BuyImportinvoiceDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("BuyImportinvoice/GetByID")]
        [HttpGet, HttpPost]
        ...
        [Route("BuyImportinvoice/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string buyimportinvoiceJson)
        {
          string errMessage = string.Empty;
          try
            {  BuyImportinvoiceContract buyimportinvoice = JsonConvert.DeserializeObject<BuyImportinvoiceContract>(buyimportinvoiceJson);           
                businessRule.RegistInstants(buyimportinvoice);
                if (buyimportinvoice.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(buyimportinvoice));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(buyimportinvoice));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("BuyImportinvoice/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = BuyImportinvoiceDFR.GetComboboxData(sessionKey,out columns,cond, out order);
