...
{
   public partial class CatDistrictController: ApiController
    {
        CatDistrictDT dta = new CatDistrictDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatDistrictBR));
        [Route("CatDistrict/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatDistrict/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatDistrictDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatDistrict/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catdistrictJson)
        {
          string errMessage = string.Empty;
          try
            {  CatDistrictContract catdistrict = JsonConvert.DeserializeObject<CatDistrictContract>(catdistrictJson);           
                businessRule.RegistInstants(catdistrict);
                if (catdistrict.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catdistrict));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catdistrict));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatDistrict/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatDistrictDFR.GetComboboxData(sessionKey,out columns,cond, out order);
