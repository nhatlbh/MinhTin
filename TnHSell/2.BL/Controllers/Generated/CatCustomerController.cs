...
{
   public partial class CatCustomerController: ApiController
    {
        CatCustomerDT dta = new CatCustomerDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatCustomerBR));
        [Route("CatCustomer/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatCustomer/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatCustomerDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatCustomer/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catcustomerJson)
        {
          string errMessage = string.Empty;
          try
            {  CatCustomerContract catcustomer = JsonConvert.DeserializeObject<CatCustomerContract>(catcustomerJson);           
                businessRule.RegistInstants(catcustomer);
                if (catcustomer.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catcustomer));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catcustomer));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatCustomer/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatCustomerDFR.GetComboboxData(sessionKey,out columns,cond, out order);
