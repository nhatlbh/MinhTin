...
{
   public partial class AdmRoleController: ApiController
    {
        AdmRoleDT dta = new AdmRoleDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(AdmRoleBR));
        [Route("AdmRole/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("AdmRole/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(AdmRoleDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("AdmRole/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string admroleJson)
        {
          string errMessage = string.Empty;
          try
            {  AdmRoleContract admrole = JsonConvert.DeserializeObject<AdmRoleContract>(admroleJson);           
                businessRule.RegistInstants(admrole);
                if (admrole.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(admrole));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(admrole));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = AdmRoleDFR.GetComboboxData(sessionKey,out columns,cond, out order);
