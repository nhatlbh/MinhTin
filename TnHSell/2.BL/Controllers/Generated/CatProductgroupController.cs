...
{
   public partial class CatProductgroupController: ApiController
    {
        CatProductgroupDT dta = new CatProductgroupDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatProductgroupBR));
        [Route("CatProductgroup/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatProductgroup/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatProductgroupDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatProductgroup/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catproductgroupJson)
        {
          string errMessage = string.Empty;
          try
            {  CatProductgroupContract catproductgroup = JsonConvert.DeserializeObject<CatProductgroupContract>(catproductgroupJson);           
                businessRule.RegistInstants(catproductgroup);
                if (catproductgroup.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catproductgroup));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catproductgroup));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatProductgroup/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatProductgroupDFR.GetComboboxData(sessionKey,out columns,cond, out order);
