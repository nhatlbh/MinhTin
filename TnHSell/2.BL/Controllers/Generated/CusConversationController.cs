...
{
   public partial class CusConversationController: ApiController
    {
        CusConversationDT dta = new CusConversationDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CusConversationBR));
        [Route("CusConversation/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CusConversation/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CusConversationDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CusConversation/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string cusconversationJson)
        {
          string errMessage = string.Empty;
          try
            {  CusConversationContract cusconversation = JsonConvert.DeserializeObject<CusConversationContract>(cusconversationJson);           
                businessRule.RegistInstants(cusconversation);
                if (cusconversation.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(cusconversation));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(cusconversation));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CusConversation/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CusConversationDFR.GetComboboxData(sessionKey,out columns,cond, out order);
