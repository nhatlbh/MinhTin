...
{
   public partial class CatProductPriceController: ApiController
    {
        CatProductPriceDT dta = new CatProductPriceDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatProductPriceBR));
        [Route("CatProductPrice/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatProductPrice/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatProductPriceDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatProductPrice/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatProductPriceDFR.GetComboboxData(sessionKey,out columns,cond, out order);
