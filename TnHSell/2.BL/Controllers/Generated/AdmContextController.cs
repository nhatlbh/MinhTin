...
{
   public partial class AdmContextController: ApiController
    {
        AdmContextDT dta = new AdmContextDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(AdmContextBR));
        [Route("AdmContext/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("AdmContext/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(AdmContextDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("AdmContext/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string admcontextJson)
        {
          string errMessage = string.Empty;
          try
            {  AdmContextContract admcontext = JsonConvert.DeserializeObject<AdmContextContract>(admcontextJson);           
                businessRule.RegistInstants(admcontext);
                if (admcontext.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(admcontext));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(admcontext));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("AdmContext/GetComboboxData")]
        [HttpGet, HttpPost]
