...
{
   public partial class CatConvresultController: ApiController
    {
        CatConvresultDT dta = new CatConvresultDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatConvresultBR));
        [Route("CatConvresult/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatConvresult/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatConvresultDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatConvresult/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catconvresultJson)
        {
          string errMessage = string.Empty;
          try
            {  CatConvresultContract catconvresult = JsonConvert.DeserializeObject<CatConvresultContract>(catconvresultJson);           
                businessRule.RegistInstants(catconvresult);
                if (catconvresult.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catconvresult));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catconvresult));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatConvresult/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatConvresultDFR.GetComboboxData(sessionKey,out columns,cond, out order);
