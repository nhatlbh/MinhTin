...
{
   public partial class CatColorController: ApiController
    {
        CatColorDT dta = new CatColorDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatColorBR));
        [Route("CatColor/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatColor/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatColorDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatColor/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catcolorJson)
        {
          string errMessage = string.Empty;
          try
            {  CatColorContract catcolor = JsonConvert.DeserializeObject<CatColorContract>(catcolorJson);           
                businessRule.RegistInstants(catcolor);
                if (catcolor.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catcolor));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catcolor));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatColor/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatColorDFR.GetComboboxData(sessionKey,out columns,cond, out order);
