...
{
   public partial class CatPaymenttypeController: ApiController
    {
        CatPaymenttypeDT dta = new CatPaymenttypeDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatPaymenttypeBR));
        [Route("CatPaymenttype/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatPaymenttype/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatPaymenttypeDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatPaymenttype/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catpaymenttypeJson)
        {
          string errMessage = string.Empty;
          try
            {  CatPaymenttypeContract catpaymenttype = JsonConvert.DeserializeObject<CatPaymenttypeContract>(catpaymenttypeJson);           
                businessRule.RegistInstants(catpaymenttype);
                if (catpaymenttype.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catpaymenttype));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catpaymenttype));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatPaymenttype/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatPaymenttypeDFR.GetComboboxData(sessionKey,out columns,cond, out order);
