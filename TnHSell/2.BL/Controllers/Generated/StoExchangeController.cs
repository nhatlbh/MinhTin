...
{
   public partial class StoExchangeController: ApiController
    {
        StoExchangeDT dta = new StoExchangeDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(StoExchangeBR));
        [Route("StoExchange/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("StoExchange/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(StoExchangeDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("StoExchange/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = StoExchangeDFR.GetComboboxData(sessionKey,out columns,cond, out order);
