...
{
   public partial class CatIocodeController: ApiController
    {
        CatIocodeDT dta = new CatIocodeDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatIocodeBR));
        [Route("CatIocode/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatIocode/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatIocodeDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatIocode/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catiocodeJson)
        {
          string errMessage = string.Empty;
          try
            {  CatIocodeContract catiocode = JsonConvert.DeserializeObject<CatIocodeContract>(catiocodeJson);           
                businessRule.RegistInstants(catiocode);
                if (catiocode.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catiocode));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catiocode));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatIocode/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatIocodeDFR.GetComboboxData(sessionKey,out columns,cond, out order);
