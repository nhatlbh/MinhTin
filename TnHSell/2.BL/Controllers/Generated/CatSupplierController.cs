...
{
   public partial class CatSupplierController: ApiController
    {
        CatSupplierDT dta = new CatSupplierDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatSupplierBR));
        [Route("CatSupplier/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatSupplier/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatSupplierDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatSupplier/Save")]
        [HttpGet, HttpPost]
        public HttpResponseMessage Save(string catsupplierJson)
        {
          string errMessage = string.Empty;
          try
            {  CatSupplierContract catsupplier = JsonConvert.DeserializeObject<CatSupplierContract>(catsupplierJson);           
                businessRule.RegistInstants(catsupplier);
                if (catsupplier.Id == 0)
                {
                    businessRule.RegistRule("Insert");
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Insert(catsupplier));
                    else
                ...
                else
                {
                    ...
                    if (businessRule.CheckRules(out errMessage))
                        return  Request.CreateResponse<string>(HttpStatusCode.OK,dta.Update(catsupplier));
                    else
                        return handleBRFailed(errMessage);
                }
            }
        ...
        [Route("CatSupplier/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatSupplierDFR.GetComboboxData(sessionKey,out columns,cond, out order);
