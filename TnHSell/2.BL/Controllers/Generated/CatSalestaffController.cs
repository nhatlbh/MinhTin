...
{
   public partial class CatSalestaffController: ApiController
    {
        CatSalestaffDT dta = new CatSalestaffDT();
        IBaseBR businessRule = BRFactory.GenerateBRObject(typeof(CatSalestaffBR));
        [Route("CatSalestaff/GetAll")]
        [HttpGet, HttpPost]
        ...
        [Route("CatSalestaff/GetGridData")]
        [HttpGet, HttpPost]
         public HttpResponseMessage GetGridData(string sessionKey,string cond = "", string order="")
        {
            try
            {
              return Request.CreateResponse<string>(HttpStatusCode.OK, JsonConvert.SerializeObject(dta.GetGridData(CatSalestaffDFR.GetGridData(sessionKey, cond, out order), order))); 
            }
            catch (Exception e)
            {
                ExceptionHandler.Log(e);
        ...
        [Route("CatSalestaff/GetComboboxData")]
        [HttpGet, HttpPost]
        public HttpResponseMessage GetComboboxData(string sessionKey,string cond="", string order="")
        {
          try
            {
                string columns = "";
                cond = CatSalestaffDFR.GetComboboxData(sessionKey,out columns,cond, out order);
