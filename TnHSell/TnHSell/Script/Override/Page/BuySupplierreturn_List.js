var buysupplierreturnList = (function () {
    ...
    var initSearchControl = function initSearchControl() {
        callService('BuyImportinvoice/GetComboboxData?sessionKey=' + sessionKey, loadCombobox, ['ddlImportinvoiceid_S', , "Tất cả"]);
    ...
    function formOpened(e) {
        ...
        if (id > 0) {
            callService(seviceName + "/GetByID?id=" + id, function (data) {
                ...
                $('#ddlImportinvoiceid').data('kendoComboBox').value(uiObj[0].ImportInvoiceID ? uiObj[0].ImportInvoiceID : '');
    ...
    var search = function search(action) {
        ...
        var importinvoiceid = $("#ddlImportinvoiceid_S").data("kendoComboBox").value();
        ...
        if (importinvoiceid != '0' && importinvoiceid != '') {
            cond += cond != '' ? ' and ' : '';
            cond += "ImportInvoiceID = " + importinvoiceid;
        }
...
{ field: 'OrderNum', title: 'Thứ tự' },
    ...
    var refreshSearchForm = function refreshSearchForm() {
        $('#txtCode_S').val('');
        $('#ddlImportinvoiceid_S').data('kendoComboBox').value('0');
