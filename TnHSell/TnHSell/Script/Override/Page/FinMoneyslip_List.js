var finmoneyslipList = (function () {
    ...
    var initSearchControl = function initSearchControl() {
        callService('CatPaymenttype/GetComboboxData?sessionKey=' + sessionKey, loadCombobox, ['ddlPaymenttypeid_S', , "Tất cả"]);
    ...
    function formOpened(e) {
        ...
        if (id > 0) {
            callService(seviceName + "/GetByID?id=" + id, function (data) {
                ...
                $('#ddlPaymenttypeid').data('kendoComboBox').value(uiObj[0].PaymentTypeID ? uiObj[0].PaymentTypeID : '');
    ...
    var search = function search(action) {
        ...
        var paymenttypeid = $("#ddlPaymenttypeid_S").data("kendoComboBox").value();
        ...
        if (paymenttypeid != '0' && paymenttypeid != '') {
            cond += cond != '' ? ' and ' : '';
            cond += "PaymentTypeID = " + paymenttypeid;
        }
    ...
    var refreshSearchForm = function refreshSearchForm() {
        $('#txtCode_S').val('');
        $('#ddlPaymenttypeid_S').data('kendoComboBox').value('0');
