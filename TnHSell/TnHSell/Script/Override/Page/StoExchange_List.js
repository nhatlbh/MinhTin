var stoexchangeList = (function () {
    ...
    var initSearchControl = function initSearchControl() {
        callService('CatStore/GetComboboxData?sessionKey=' + sessionKey, loadCombobox, ['ddlFromstoreid_S', , "Tất cả"]);
        callService('CatStore/GetComboboxData?sessionKey=' + sessionKey, loadCombobox, ['ddlTostoreid_S', , "Tất cả"]);
    ...
    function formOpened(e) {
        ...
        if (id > 0) {
            callService(seviceName + "/GetByID?id=" + id, function (data) {
                ...
                $('#ddlFromstoreid').data('kendoComboBox').value(uiObj[0].FromStoreID ? uiObj[0].FromStoreID : '');
                $('#ddlTostoreid').data('kendoComboBox').value(uiObj[0].ToStoreID ? uiObj[0].ToStoreID : '');
    ...
    var search = function search(action) {
        ...
        var fromstoreid = $("#ddlFromstoreid_S").data("kendoComboBox").value();
        var tostoreid = $("#ddlTostoreid_S").data("kendoComboBox").value();
        ...
        if (fromstoreid != '0' && fromstoreid != '') {
            cond += cond != '' ? ' and ' : '';
            cond += "FromStoreID = " + fromstoreid;
        }
        if (tostoreid != '0' && tostoreid != '') {
            cond += cond != '' ? ' and ' : '';
            cond += "ToStoreID = " + tostoreid;
        }
    ...
    function bindGrid(data) {
        ...
        var grid = $('#grid').kendoGrid({
            ...
            { field: 'Code', title: 'Mã' },
            { field: 'Cat_Store_Name_From', title: 'Từ kho' },
            { field: 'Cat_Store_Name_To', title: 'Tới kho' },
    ...
    var refreshSearchForm = function refreshSearchForm() {
        $('#txtCode_S').val('');
        $('#ddlFromstoreid_S').data('kendoComboBox').value('0');
        $('#ddlTostoreid_S').data('kendoComboBox').value('0');
