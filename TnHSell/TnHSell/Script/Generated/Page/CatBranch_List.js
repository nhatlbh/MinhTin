var catbranchList = (function () {
    var seviceName = 'CatBranch';
    ...
    sessionKey = $('#hdfSessionKey').val();
            callService('CatBranch/GetComboboxData?sessionKey='+sessionKey, loadCombobox, ['ddlParentbranchid_S', , "Tất cả"]);
...
$('#ddlParentbranchid').data('kendoComboBox').value(uiObj[0].ParentBranchID ? uiObj[0].ParentBranchID : '');
...
$('#txtOrdernum').val(numberFormat(uiObj[0].OrderNum));
        ...
        else
        {
            ...
            $('#txtCode').val('');
$('#txtName').val('');
$('#ddlParentbranchid').data('kendoComboBox').value('');
$('#txtDescription').val('');
$('#txtAddress').val('');
$('#txtPhone').val('');
$('#txtOrdernum').val('');
...
var parentbranchid = $("#ddlParentbranchid_S").data("kendoComboBox").value();
if (code != null && code != '') {
                                                                    cond += cond != '' ? ' and ' : '';
...
if (parentbranchid != '0' && parentbranchid != '') {
                                                                cond += cond != '' ? ' and ' : '';
                                                                cond += "ParentBranchID = " + parentbranchid;
                                                            }
...
{ field: 'Code', title: 'Mã'},
{ field: 'Name', title: 'Tên chi nhánh'},
{ field: 'Cat_Branch_Name', title: 'Chi nhánh cha'},
...
{ field: 'OrderNum', title: 'Thứ tự'},
    ...
    var refreshSearchForm = function refreshSearchForm() {
        $('#txtCode_S').val('');
$('#txtName_S').val('');
$('#ddlParentbranchid_S').data('kendoComboBox').value('0');
    }
