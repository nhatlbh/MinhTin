...
var catguaranteestatusForm = (function () {
    var seviceName = 'CatGuaranteestatus';
...
Ordernum: getNumber( $('#txtOrdernum').val())
  ...
  var validate = function () {
    ...
    var save = function save(ftnAfterSave) {
        var data = buildDTO();
        if (validate()) {
            callService(seviceName + '/Save?catguaranteestatusJson=' + JSON.stringify(data), function (id) {
                setFormId(id);
                if(ftnAfterSave)
                    ftnAfterSave();
            });
    ...
    var del = function del(ftnAfterDelete) {
        var id = $('#hdfId').val();
        if (id == '' || id == '0') {
            alert('Chưa chọn Trạng thái bảo hành cần xóa');
        }
        else if (confirm("Bạn có muốn xóa Trạng thái bảo hành này?")) {
