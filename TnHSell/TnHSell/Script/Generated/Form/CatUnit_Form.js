...
var catunitForm = (function () {
    var seviceName = 'CatUnit';
...
Ordernum: getNumber( $('#txtOrdernum').val())
  ...
  var validate = function () {
    ...
    var save = function save(ftnAfterSave) {
        var data = buildDTO();
        if (validate()) {
            callService(seviceName + '/Save?catunitJson=' + JSON.stringify(data), function (id) {
                setFormId(id);
                if(ftnAfterSave)
                    ftnAfterSave();
            });
    ...
    var del = function del(ftnAfterDelete) {
        var id = $('#hdfId').val();
        if (id == '' || id == '0') {
            alert('Chưa chọn Đơn vị tính cần xóa');
        }
        else if (confirm("Bạn có muốn xóa Đơn vị tính này?")) {
