...
var catmanagementgroupForm = (function () {
    var seviceName = 'CatManagementgroup';
...
Ordernum: getNumber( $('#txtOrdernum').val())
  ...
  var validate = function () {
    ...
    var save = function save(ftnAfterSave) {
        var data = buildDTO();
        if (validate()) {
            callService(seviceName + '/Save?catmanagementgroupJson=' + JSON.stringify(data), function (id) {
                setFormId(id);
                if(ftnAfterSave)
                    ftnAfterSave();
            });
    ...
    var del = function del(ftnAfterDelete) {
        var id = $('#hdfId').val();
        if (id == '' || id == '0') {
            alert('Chưa chọn Nhóm quản lý cần xóa');
        }
        else if (confirm("Bạn có muốn xóa Nhóm quản lý này?")) {
