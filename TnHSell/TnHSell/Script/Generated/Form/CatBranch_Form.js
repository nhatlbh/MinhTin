...
var catbranchForm = (function () {
    var seviceName = 'CatBranch';
    var sessionKey = $('#hdfSessionKey').val();
    var initFormControl = function initFormControl(){
            sessionKey = $('#hdfSessionKey').val();
             numberOnly($('#hdfId'))
      callService('CatBranch/GetComboboxData?sessionKey='+sessionKey, loadCombobox, ['ddlParentbranchid']);
...
Code: $('#txtCode').val(),
Name: $('#txtName').val(),
Parentbranchid: $("#ddlParentbranchid").data("kendoComboBox").value(),
...
Ordernum: getNumber( $('#txtOrdernum').val())
  ...
  var validate = function () {
    ...
    var save = function save(ftnAfterSave) {
        var data = buildDTO();
        if (validate()) {
            callService(seviceName + '/Save?catbranchJson=' + JSON.stringify(data), function (id) {
                setFormId(id);
                if(ftnAfterSave)
                    ftnAfterSave();
            });
